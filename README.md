...
JunkBranding の公式サイトです。

茨城を拠点に、東京・神奈川・全国オンライン対応で活動する小規模ブランディング / Web制作スタジオとして、ブランドの「らしさ」を Web、言葉、ビジュアル、動きで伝えることを目的に設計しています。

このリポジトリは、単なるポートフォリオではなく、以下を同時に担う本番サイトです。

- ブランド体験を伝える表現サイト
- 事業内容と制作姿勢を説明する営業サイト
- 実績、料金、問い合わせ導線を持つリード獲得サイト
- SEO、構造化データ、AI向け情報整備まで含む運用サイト

## このサイトで重視していること

- 小さなスタジオらしい丁寧さが伝わること
- 第一印象だけで終わらず、問い合わせまで自然につながること
...
### 3. 問い合わせ導線まで含めた実運用サイト

お問い合わせは見た目だけでなく、送信処理まで実装しています。
...
各ページに metadata と JSON-LD を持たせ、さらに AI 向けの `llms.txt` も返しています。
...
現在、必須なのは問い合わせメール送信に使う Resend のキーです。
...
- 環境変数が未設定だと、サーバー起動時に問い合わせ処理でエラーになります
- ローカルで問い合わせ送信を確認したい場合も設定が必要です
...
このプロジェクトは Next.js の標準的な Node 実行が前提です。最も相性が良いのは Vercel ですが、VPS やクラウドサーバーでも運用できます。

### 1. Vercel にデプロイする場合

最も簡単です。Next.js App Router、metadata、route handler、Analytics と相性が良いため、基本的にはこれを推奨します。
...
### 2. VPS / さくら / ConoHa / AWS EC2 などにアップする場合

Node.js が動くサーバーであれば運用できます。静的 HTML ではなく Next.js のアプリとして起動します。
...
本番では、`pnpm start` を直接叩きっぱなしにするのではなく、プロセスマネージャを使うのが前提です。
...
### 3. 静的アップロードだけで済ませられるか

このプロジェクトは問い合わせ送信や route handler を持っているため、単純な静的 HTML 書き出し前提のサイトではありません。

つまり:

- FTP で HTML を置くだけの運用にはそのままでは向きません
- Node.js 実行環境を持つホスティングか、Vercel のような実行基盤が必要です

## サーバーへアップする時の実務フロー

Vercel 以外のサーバーへデプロイする場合の、現実的な流れです。
...
- 事業概要、主要ページ、説明方針を AI が読みやすい形で持つ
...
Server Actions と環境変数が絡むため、ホスティング条件を必ず確認します。

## 現在の実装上の補足

- `next.config.mjs` では `typescript.ignoreBuildErrors` を有効にしています
- これは build を通しやすくする反面、型エラーを本番ビルドが止めない設定です
- 安全性を高めるなら、`pnpm exec tsc --noEmit` を運用に必ず含めてください
...
このリポジトリは JunkBranding サイト本体の実装です。公開方法や二次利用の扱いは、運用ポリシーに合わせて別途管理してください。
