...
- PageSpeedだけを軽くする分岐は入れない。実ユーザーにも同じ改善が効く形にする
...
- TOP下層セクション: `components/pages/home/home-deferred-sections.tsx` をdynamic importし、HERO初期表示のJSから分離
- GTM: `app/layout.tsx` で `load` 後の `requestIdleCallback` まで遅延。計測タグより初期操作を優先
- SoundToggle / CookieConsent: 全ユーザー共通でidle後に読み込み。初期表示直後には出さない
- CustomCursor / FloatingParticles / RisingRainbowOrb / BottomHeatHaze: PCでもidle後に読み込み。SPでは出さない
- Loading: SPはfast startでローディング演出と音声選択待ちを短縮

### OffscreenCanvas

- `components/effects/floating-particles.tsx`: 対応ブラウザでは `floating-particles.worker.ts` に描画と粒子計算を移す
- `components/effects/top-canvas-film-overlay.tsx`: 対応ブラウザでは `top-canvas-film-overlay.worker.ts` に全画面フィルム演出を移す
- 非対応ブラウザでは従来のメインスレッドCanvas描画にfallback
- `ScatterText` はDOM文字位置計測が必要なため、Worker化よりHTMLテキスト + Canvas描画の軽量化を優先
