...
const MIST_ORBS = [
  ...
  },
  {
    ...
    filter: 'blur(30px)',
    animationName: 'mistFloat3',
  },
] satisfies Array<CSSProperties & { animationName: string }>
...
const contentTransition = 'opacity 0.42s cubic-bezier(0.16, 1, 0.3, 1), transform 0.42s cubic-bezier(0.16, 1, 0.3, 1), filter 0.42s cubic-bezier(0.16, 1, 0.3, 1)'
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden="true"
      >
        {MIST_ORBS.map(({ animationName, ...orbStyle }, index) => (
          <div
            key={animationName}
            style={{
              ...orbStyle,
              position: 'absolute',
              height: orbStyle.width,
              maxHeight: orbStyle.maxWidth,
              borderRadius: '50%',
              animation: isTransitioning
                ? `${animationName} ${index === 1 ? 4 : index === 2 ? 3.5 : 3}s ease-in-out infinite`
                : 'none',
            }}
          />
        ))}
      ...
      <div 
        style={{
          ...
          transition: isMobile ? 'opacity 0.2s ease' : contentTransition,
          position: 'relative',
          zIndex: 10,
