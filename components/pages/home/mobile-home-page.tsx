...
const mobileServices = [
  {
    ...
    titleJa: 'Webデザイン',
    description:
      'ブランドの世界観とユーザー導線を両立したWebサイトを設計します。',
  },
  {
    title: 'Branding',
    titleJa: 'ブランディング',
    description:
      'ロゴ、カラー、言葉、トーンを整理し、らしさが伝わる形へ整えます。',
  },
  {
    title: 'Development',
    titleJa: '開発・実装',
...
export function MobileHomePage() {
  return (
    <div className="bg-transparent">
      <section className="min-h-svh px-6 py-28 flex flex-col justify-center">
      ...
      <section className="px-6 py-20 glass-card">
        ...
        <div className="mt-10 space-y-5">
          {mobileServices.map(service => (
            <article
              key={service.title}
              className="rounded-3xl bg-background/55 p-6 rainbow-border"
            >
              <p className="type-eyebrow text-3xl text-foreground">
                {service.title}
              </p>
              <h3 className="type-card-title mt-2 text-lg text-primary">
                {service.titleJa}
              </h3>
              <p className="type-body-compact mt-4 text-sm text-muted-foreground">
                {service.description}
              </p>
            </article>
          ))}
      ...
      <section className="px-6 py-20 glass-light">
        <p className="type-eyebrow text-5xl text-foreground/45">
          {homeWorksPreview.eyebrow}
        </p>
        <h2 className="type-section-title mt-6 text-3xl gradient-text">
      ...
      <section className="px-6 py-20 glass-card">
        ...
        <p className="type-hero-title mt-4 text-4xl gradient-text">
          {homeArea.area}
        </p>
        <p className="type-body mt-6 text-base text-muted-foreground">
